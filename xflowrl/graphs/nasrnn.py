...
def nas_node(graph, input_list, x):
    t = list()
    for i in range(8):
        t.append(combine(graph, x, input_list))
    midt = list()
...
if __name__ == '__main__':
    built_graph = build_graph_nasrnn()
    onnx_model = ts.export_onnx(built_graph)
    onnx.save(onnx_model, 'nasrnn.onnx')
